...
defmodule Astarte.FDO.OwnershipVoucher.LoadRequest do
  ...
  defp verify_owner_key_matches(changeset) do
    ...
    %Astarte.Secrets.Key{name: key_name, public_pem: pem} = fetch_field!(changeset, :extracted_owner_key)

    with {:ok, %PublicKey{encoding: :x509, body: voucher_der}} <-
           OVCore.entry_private_key(List.last(voucher_struct.entries)),
         [{_type, key_der, :not_encrypted}] <- :public_key.pem_decode(pem),
         true <- voucher_der == key_der do
      changeset
    else
      false ->
        Logger.warning(
          "LoadRequest: key_name \"#{key_name}\" was found in the secrets store " <>
            "but its public key DER bytes do not match the public key in the voucher's last entry. " <>
  ...
  defp pubkey_type_to_algorithm(_), do: :error
end
