...
defmodule Astarte.Secrets.OwnerKeyInitialization do
  ...
  defp do_upload_key(key_name, key_algorithm, key_body, namespace) do
    # need to check if a key with this name has already been uploaded/created;
    # in this case the OpenBao import API is not idempotent and will return error 500
    case Secrets.get_key(key_name, namespace: namespace) do
      {:ok, %{name: ^key_name}} ->
        {:ok, "A key with the name #{key_name} has already been imported"}
