...
defmodule Astarte.DataUpdaterPlant.DataUpdater.Core.KeyAgreement.ExchangeFailedTest do
  ...
  describe "new/3" do
    test "creates a struct with a valid reason" do
      msg = ExchangeFailed.new(0, :hash_mismatch, "hash did not match")
      assert msg.seq_num == 0
      assert msg.reason == :hash_mismatch
      assert msg.error_msg == "hash did not match"

      assert ExchangeFailed.new(1, :invalid_argument, "bad key").reason == :invalid_argument

      assert ExchangeFailed.new(2, :internal_server_error, "crash").reason ==
               :internal_server_error

      assert ExchangeFailed.new(3, :unprocessable_entity, "key mismatch").reason ==
    ...
    test "raises for unknown atoms" do
      for reason <- [:some_random_internal_error, :key_type_mismatch] do
        assert_raise ArgumentError, fn ->
          ExchangeFailed.new(0, reason, "msg")
        end
      end
    end
  end
  ...
  describe "cbor_encode/1 and cbor_decode/1 round-trip" do
    test "round-trips successfully for all valid reasons" do
      for reason <- [
        ...
        original = ExchangeFailed.new(1, reason, "some context")
  ...
  describe "encode/1" do
    test "returns a three-element list [seq_num, code, error_msg]" do
      assert ExchangeFailed.encode(ExchangeFailed.new(0, :internal_server_error, "crash")) == [
               0,
               0,
               "crash"
             ]

      assert ExchangeFailed.encode(ExchangeFailed.new(1, :invalid_argument, "bad key")) == [
               1,
               1,
               "bad key"
             ]

      assert ExchangeFailed.encode(ExchangeFailed.new(2, :hash_mismatch, "mismatch")) == [
               2,
               2,
               "mismatch"
             ]

      assert ExchangeFailed.encode(ExchangeFailed.new(3, :unprocessable_entity, "bad alg")) == [
               3,
               3,
               "bad alg"
             ]
