  ...
  * `/keyAgreement/4` - Receives an ExchangeFailed from the device signalling a key-agreement failure.
  ...
  def handle_control(state, "/keyAgreement/4", payload, timestamp) do
    ...
    case ExchangeFailed.cbor_decode(payload) do
      {:ok, %ExchangeFailed{seq_num: seq_num, reason: reason, error_msg: error_msg}} ->
        Logger.warning(
  ...
  defp process_key_agreement(
    ...
    with {:ok, exchange_resp} <- decode_exchange_resp(state, payload, timestamp, data.key_type),
      ...
      {:error, reason} ->
        ...
        {exchange_failed_reason, exchange_failed_msg} = classify_exchange_resp_error(reason)
        ...
        :ok =
          send_exchange_failed(
            ...
            state.device_id,
            0,
            exchange_failed_reason,
            exchange_failed_msg
          )

        {:ok, failed_key_state} =
          HandshakeState.transition(state.encrypted_endpoints_key, {:error, reason})
  ...
  @spec classify_exchange_resp_error(
          :invalid_payload
          | :invalid_cose_key
          | :key_type_mismatch
          | :key_mismatch_or_unsupported
          | {:ecdh_failed, binary()}
          | :invalid_transition
          | term()
        ) :: {atom(), String.t()}
  defp classify_exchange_resp_error(reason) do
    case reason do
      :invalid_payload -> {:invalid_argument, "invalid payload"}
      :invalid_cose_key -> {:invalid_argument, "invalid COSE key"}
      :key_type_mismatch -> {:unprocessable_entity, "unsupported key type"}
      :key_mismatch_or_unsupported -> {:unprocessable_entity, "unsupported or mismatched key"}
      {:ecdh_failed, _} -> {:internal_server_error, "key derivation failed"}
      _ -> {:internal_server_error, "unexpected error"}
    end
  end
  ...
  defp send_exchange_failed(realm, device_id, seq_num, reason, error_msg) do
    ...
    exchange_failed = ExchangeFailed.new(seq_num, reason, error_msg)
