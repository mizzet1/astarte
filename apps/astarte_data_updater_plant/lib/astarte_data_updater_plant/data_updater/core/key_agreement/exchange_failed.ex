...
defmodule Astarte.DataUpdaterPlant.DataUpdater.Core.KeyAgreement.ExchangeFailed do
  ...
  """
  @spec new(non_neg_integer(), reason(), String.t()) :: t()
  def new(seq_num, reason, error_msg)
    ...
    case Ecto.Type.cast(@reasons, reason) do
      {:ok, valid_reason} ->
        %ExchangeFailed{seq_num: seq_num, reason: valid_reason, error_msg: error_msg}

      _error ->
        raise ArgumentError, "invalid ExchangeFailed reason: #{inspect(reason)}"
    end
  end
