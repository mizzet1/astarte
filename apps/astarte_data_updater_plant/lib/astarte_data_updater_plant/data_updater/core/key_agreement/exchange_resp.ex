...
defmodule Astarte.DataUpdaterPlant.DataUpdater.Core.KeyAgreement.ExchangeResp do
  ...
  Ggenerates a fresh ephemeral key pair using the same suite (`key_type`) that the remote side
  ...
  def new(%InitExchange{seq_num: seq_num, key_type: key_type}) do
    key = generate_key(key_type)
    %__MODULE__{seq_num: seq_num, public_key: key}
  end
  ...
  defp parse([seq_num, public_key]) when is_integer(seq_num) and seq_num >= 0 do
    with {:ok, cose_key_bytes} <- unwrap_bytes(public_key),
         {:ok, cose_key_map} <- decode_cbor(cose_key_bytes),
         {:ok, cose_key} <- decode_cose_key(cose_key_map) do
      {:ok, %__MODULE__{seq_num: seq_num, public_key: cose_key}}
    end
  end

  defp parse(_), do: {:error, :invalid_payload}

  # Accept any supported EC key type
  defp decode_cose_key(cose_key_map) do
    case COSE.Keys.decode(cose_key_map) do
      {:ok, %OKP{crv: :x25519} = key} -> {:ok, key}
      {:ok, %ECC{crv: :p256} = key} -> {:ok, key}
      {:ok, _} -> {:error, :unsupported_key_type}
      {:error, _} -> {:error, :invalid_cose_key}
    end
  end
