...
defmodule Astarte.PairingWeb.Controllers.OwnerKeyControllerTest do
  ...
  describe "/fdo/owner_key" do
    ...
    test "returns an info message if the key is already existing", context do
      %{
        auth_conn: conn,
        create_path: path,
        create_key_payload: payload
      ...
      payload_new_key_name =
        update_in(payload, [:data, :key_name], fn _ -> "duplicated_key" end)

      conn
      |> post(path, payload_new_key_name)
      |> response(200)

      key_recreation_output =
        conn
        |> post(path, payload_new_key_name)
        |> response(200)

      assert key_recreation_output =~ "has already been created"
    end
